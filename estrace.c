...
// Modified for Android by YourName 2023-MM-DD
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
#include <argp.h>
...
int main(int argc, char **argv)
{
    ...
    obj = estrace_bpf__open_opts(&open_opts);
    if (!obj) {
    ...
    bpf_program__set_autoload(obj->progs.tracepoint__raw_syscalls__sys_enter, true);
    bpf_program__set_autoload(obj->progs.tracepoint__raw_syscalls__sys_exit, true);

    err = estrace_bpf__load(obj);
    if (err) {
        fprintf(stderr, "failed to load BPF object: %d\n", err);
        goto cleanup;
    }

    err = estrace_bpf__attach(obj);
    if (err) {
